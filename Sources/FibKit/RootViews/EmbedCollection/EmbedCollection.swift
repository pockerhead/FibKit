...
extension EmbedCollection: FibViewHeader {
    ...
    public func configure(with data: ViewModelWithViewClass?) {
		...
		pageControl.numberOfPages = ((data.provider as? FibKit.FibGridHeaderProvider)?.sections.first as? FibKit.FibGridProvider)?.dataSource.data.count ?? data.provider?.numberOfItems ?? 0 //data.provider?.numberOfItems ??
