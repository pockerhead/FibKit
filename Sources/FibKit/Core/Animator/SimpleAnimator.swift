...
open class SimpleAnimator: Animator {
	...
	open override func insert(collectionView: FibGrid, view: UIView, at: Int, frame: CGRect) {
		...
		Task {
			if await collectionView.isReloading, await collectionView.hasReloaded, collectionView.bounds.intersects(frame) {
				hide(view: view)
				animate {
					self.show(view: view)
				}
	...
	open override func delete(collectionView: FibGrid, view: UIView) {
		Task {
			if await collectionView.isReloading, collectionView.bounds.intersects(view.frame) {
				animate({
					self.hide(view: view)
				}, completion: { _ in
					Task {
						if await !collectionView.visibleCells.contains(view) {
