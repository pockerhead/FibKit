...
struct RevertedLayoutHelper {
    ...
    -> [CGRect] where SizeArray.Iterator.Element == CGSize {
        ...
        for cellSize in sizes {
            ...
            switch alignItems {
            case .start:
                cellFrame = CGRect(origin: CGPoint(x: offset, y: secondaryRange.upperBound), size: cellSize)
            case .end:
                cellFrame = CGRect(origin: CGPoint(x: offset,
                                                   y: secondaryRange.lowerBound + cellSize.height),
                                   size: cellSize)
            case .center:
                let secondaryOffset = secondaryRange.upperBound -
                    (secondaryRange.lowerBound + secondaryRange.upperBound + cellSize.height) / 2
                cellFrame = CGRect(origin: CGPoint(x: offset, y: secondaryOffset),
                                   size: cellSize)
            case .stretch:
                cellFrame = CGRect(origin: CGPoint(x: offset, y: secondaryRange.upperBound),
                                   size: CGSize(width: cellSize.width,
                                                height: secondaryRange.lowerBound + secondaryRange.upperBound))
            }
            frames.append(cellFrame)
            offset -= cellSize.width + spacing
        }
    ...
    static func distribute(maxY: CGFloat,
                           justifyContent: JustifyContent,
                           maxPrimary: CGFloat,
                           totalPrimary: CGFloat,
                           minimunSpacing: CGFloat,
                           numberOfItems: Int) -> (offset: CGFloat, spacing: CGFloat) {
        var offset: CGFloat = maxY
        var spacing = minimunSpacing
        guard numberOfItems > 0 else { return (offset, spacing) }
        if totalPrimary + CGFloat(numberOfItems - 1) * minimunSpacing < maxPrimary {
            let leftOverPrimary = maxPrimary - totalPrimary
