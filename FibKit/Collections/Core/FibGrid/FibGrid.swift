...
final class GridsReuseManager {
    ...
    var grids = [String: WeakRef<FibGrid>]()
    var dummyViews = [String: UIView]()
    private lazy var timer: Timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) {[weak self] _ in
        DispatchQueue.global(qos: .background).async {
            self?.grids = self?.grids.filter({ $0.value.ref != nil }) ?? [:]
        }
    }
...
final public class FibGrid: CollectionView {
    ...
    /// declarative described sections with items
    /// when sets, called reload
...
//    func displayEmptyView(model: DITCore.InfoMessageView.ViewModel, height: CGFloat? = nil, animated: Bool) {
...
//    }
    ...
    private func reloadSections(_ oldValue: [GridSection]) {
        ...
        if !forcedProviderUpdate,
           let provider = self.provider as? FibGridHeaderProvider {
            ...
            let provider = FibGridHeaderProvider(identifier: "RootProvider",
                                                 layout: newLayout,
                                                 animator: animated ? AnimatedReloadAnimator() : nil,
                                                 sections: self.sections,
                                                 collectionView: self)
                .bindReload({[weak self] in
    ...
    private func nestedInteractiveViews(in view: UIView, contain point: CGPoint) -> Bool {
        ...
        if let formView = view as? FibGrid,
            ...
            if view is FibGrid == false, view.isPotentiallyInteractive, view.bounds.contains(convert(point, to: view)) {
                return true
            }
...
extension FibGrid: UIViewRepresentable {
    ...
    public convenience init(@SectionBuilder _ sections: () -> [GridSection],
