...
open class FibGridProvider: ItemProvider, CollectionReloadable, LayoutableProvider {
    ...
    @discardableResult
    func bindReload(_ didReload: (() -> Void)?) -> FibGridProvider {
    ...
    public func didTap(view: UIView, at: Int) {
        if let tapHandler = tapHandler {
            if separatorViewModel != nil {
                if at % 2 == 0 {
                    let context = TapContext(grid: self.collectionView as? FibGrid, view: view as! ViewModelConfigurable, index: at / 2, dataSource: dataSource)
                    tapHandler(context)
                }
                return
            }
            let context = TapContext(grid: self.collectionView as? FibGrid, view: view as! ViewModelConfigurable, index: at, dataSource: dataSource)
            tapHandler(context)
        }
    }
    ...
    public func didLongTapEnded(context: LongGestureContext) {
        guard let finalIndex = context.lastReorderedIndex?.softClamp(0, dataSource.data.count - 1) else {
            return
        }
            ...
            if var separatorViewModel = separatorViewModel {
                if index % 2 != 0 {
                    ...
                    return sizeSource.size(at: index,
                                           ...
                                           dummyView: viewSource.getDummyView(data: separatorViewModel) as! ViewModelConfigurable,
